...
async def get_embed_colour(guild_id):
    try:
        # Log the type and value of guild_id
        logger.debug(f"Type of guild_id: {type(guild_id)}, Value of guild_id: {guild_id}")

        # Convert guild_id to an integer
        guild_id = int(guild_id)

        async with aiosqlite.connect(db_path) as conn:
            cursor = await conn.execute('SELECT value FROM customisation WHERE type = ? AND guild_id = ?', ("embed_color", guild_id))
            row = await cursor.fetchone()
            if row:
                return int(row[0], 16)  # Convert the hex string to an integer
            else:
                return 0xc4a7ec  # Default color if not set
        ...
        async with aiosqlite.connect(db_path) as conn:
            try:
                ...
                cursor = await conn.execute(
                    ...
                    ("embed_color", interaction.guild_id))
